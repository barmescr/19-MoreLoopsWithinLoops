...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 80
    answer = largest_number(([1, 2, 3, 4, 5, 6, 7, 8, 9], [], [80, 79, -80]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for m in range(len(seq_seq)):
        if len(seq_seq[m]) > 0:
            if seq_seq[m][0] > largest:
                largest = seq_seq[m][0]
        for j in range(len(seq_seq[m])):
            if seq_seq[m][j] > largest:
                largest = seq_seq[m][j]
    return largest
...
def run_test_largest_negative_number():
    ...
    # Test 4:
    expected = -2
    answer = largest_number(([-10, -20, -25], [-2, -4, -6, -8, -10], [-10]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for g in range(len(seq_seq)):
        for f in range(len(seq_seq[g])):
            if seq_seq[g][f] < 0:
                largest = seq_seq[g][f]
    for m in range(len(seq_seq)):
        if len(seq_seq[m]) > 0:
            if 0 > seq_seq[m][0] > largest:
                largest = seq_seq[m][0]
        for j in range(len(seq_seq[m])):
            if 0 > seq_seq[m][j] > largest:
                largest = seq_seq[m][j]
    return largest
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(seq_seq[0])):
        test = test + [seq_seq[0][k]]
    for j in range(len(test)):
        for z in range(1, len(seq_seq)):
            for x in range(len(seq_seq[z])):
                if test[j] == seq_seq[z][x]:
                    return True
    return False
